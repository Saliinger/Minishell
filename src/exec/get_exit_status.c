...
/**
 * brief : this version waits for any child to die, if the last child died, 
 * 											returns by pointer it's exit status
 * 
 */
static int	wait_get_child_err(pid_t last_child_pid, \
								int *last_child_exit_status, bool *jump_sigint)
{
	pid_t	got_pid;
	int		status;
...
/**
 * brief : waits for all children to die, returns the exit status 
 * 														of the last child
...
/**
 * brief : waits for all children to die, if != 0, returns the exit status 
 * 																of the last child
	...
	if (last_pid > 0)
	{
		...
		if (exit_status == -1)
			return (free_pids(pids_list), \
						printerr("%s:%d: err: exit_status= %d", \
									__FILE__, __LINE__, exit_status), ERR_PRIM);
		if (exit_status >= 0)
 ...
 * (CMD_BUILTIN) will check if should wait for potential children (p.pids_list)
 * 										, then return the builtin_exit_status 
 * (CMD_EXTREN) should wait for children (p.pids_list),
 * 							 extracting ir from last child created (p.pid_last)
 */
int	get_exit_status(t_infos *inf)
{
	int	exit_status;
