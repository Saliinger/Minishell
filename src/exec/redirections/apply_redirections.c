...
/*   Updated: 2024/12/13 07:32:42 by ekrebs           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * brief : does the heredoc redir (creates a pipe, 
 * writes the heredoc in it's WRITE end, replaces the STDIN with it's READ end)
 * 
 */
int	do_heredoc_redir(t_command_exec *cmd)
{
	...
	if (cmd->last_heredoc_str)
		err = write(herepipe[WE], cmd->last_heredoc_str, \
											ft_strlen(cmd->last_heredoc_str));
	if (err == -1)
		return (perror("write in herepipe WRITE end"), \
						ft_close(&herepipe[WE]), ft_close(&herepipe[RE]), ERR);
...
/**
 * brief : applies the redir IN and OUT from files 
 * 							(<infile, <<heredoc, >outfile, >>outfile_append)
...
static int	apply_files_redirs(t_command_exec *cmd, int *fd_redir)
{
	...
	if (fd_redir[IN] != -1)
	{
		if (fd_redir[IN] == HEREDOC_FILENO || \
							fd_redir[IN] == HEREDOC_QUOTES_FILENO)
		{
...
/**
 * brief : applies the redirections for the pipes and <, <<, >, >> 
 * 					redirections acording to the infos contained in cmd
