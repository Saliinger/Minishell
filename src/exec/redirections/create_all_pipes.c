...
/*   Updated: 2024/12/13 07:29:59 by ekrebs           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_close_pipes(int pipe_count, int ***pipes_tab_adr)
{
	...
	while (i < pipe_count)
	{
		if (pipes[i])
		{
			...
			if (err)
				return (printerr("%s: %d: err closing pipes", \
...
int	create_all_pipes(int cmd_count, int ***pipes_out)
{
	...
	while (i < cmd_count - 1)
	{
		...
		if (!pipes[i])
			return (perror("#creating pipes: malloc"), \
												ft_close_pipes(i, &pipes), ERR);
		if (pipe(pipes[i]) == -1)
			return (perror("#creapting pipes: pipe"), \
											ft_close_pipes(i + 1, &pipes), ERR);
		i++;
	}
	*pipes_out = pipes;
	return (EXIT_SUCCESS);
