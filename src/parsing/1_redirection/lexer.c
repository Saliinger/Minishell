...
char **add_redir(char **tab, char **to_add)
{
    ...
    res = (char **)malloc(sizeof(char *) * (nbr_of_line(tab) + nbr_of_line(to_add) + 1));
    if (!res)
...
void extend_get_redir(char *line, int *start, int *end)
{
    ...
    {
        (*end)++;
        while (line[*start + *end] && (line[*start + *end] == '<' || line[*start + *end] == '>'))
            (*end)++;
        return;
    }
    ...
    {
        while (line[*start + *end] && line[*start + *end] != '<' && line[*start + *end] != '>' && status == 0)
        {
            status = in_quote(status, line[*start + *end]);
            (*end)++;
        }
